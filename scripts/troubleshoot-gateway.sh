...
# 1. Stop any userspace proxies so Linux Kernel Transparent Layer-3 Routing handles 100% of game traffic
systemctl stop haproxy rinetd wirenet-gateway 2>/dev/null || true
systemctl disable haproxy rinetd 2>/dev/null || true

# 2. Allow forwarding unconditionally to/from wg0
...
# 3. Outbound internet MASQUERADE only on default public interface (Preserves Real Player Source IPs across wg0!)
...
# 4. Pure Transparent Layer-3 DNAT: Forwards game ports 25565-25700 directly to Node with real player source IP intact
...
echo "  [✓] Pure Transparent Layer-3 Routing & Firewall Active."
